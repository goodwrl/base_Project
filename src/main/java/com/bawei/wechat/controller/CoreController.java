package com.bawei.wechat.controller;

import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import com.bawei.wechat.util.ReturnModel;
...
public class CoreController extends GenericController {
     ...
     * 详情请见: https://mp.weixin.qq.com/wiki?action=doc&id=mp1421141115&t=0.8682137792547111#4
     *
     * @param response
     * @param url     当前网页的URL，不包含#及其后面部分
     */
    @RequestMapping(value = "getJsapiSignature")
    public void getJsapiSignature(HttpServletResponse response, @RequestParam("url") String url) {
        ReturnModel returnModel = new ReturnModel();
        try {
            WxJsapiSignature wxJsapiSign = wxMpService.createJsapiSignature(url);
            returnModel.setResult(true);
            returnModel.setDatum(wxJsapiSign);
            renderString(response, returnModel);
        } catch (WxErrorException e) {
            returnModel.setResult(false);
            returnModel.setReason(e.getError().toString());
